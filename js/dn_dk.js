  ...
  try {
    ...
    if (foundUser) {
      ...
      if (foundUser.role === "admin" || foundUser.role === "Admin") {
        ...
        foundUser.role === "nhanvien" ||
        foundUser.role === "Nhân Viên"
      ) {
...
function updateHeader() {
  ...
  if (currentUser) {
    ...
    const myOrdersItem = document.querySelector(
      '.yes_acc a[href="my-orders.html"]'
    );
    if (myOrdersItem) {
...
// 1. Hàm ẩn hiện ô nhập liệu dựa trên vai trò
function toggleFields() {
  // Lấy vai trò đang chọn
  const role = document.querySelector(
    'input[name="accountType"]:checked'
  ).value;

  // Lấy 2 thẻ div chứa ô nhập riêng
  const divAddress = document.getElementById("field-address");
  const divLicense = document.getElementById("field-license");
  ...
  // Logic hiển thị
  if (role === "KhachHang") {
    divAddress.style.display = "block"; // Khách cần địa chỉ
  } else if (role === "Shipper") {
    divLicense.style.display = "block"; // Shipper cần biển số
  }
  // Nhân viên nhà hàng tạm thời không cần thêm gì đặc biệt ở form này
}

// 2. Hàm Đăng Ký (Gọi API)
async function register() {
  ...
  const confirmPass = document.getElementById("mk2").value;
  const role = document.querySelector(
    'input[name="accountType"]:checked'
  ).value;
  ...
  // Chuẩn bị dữ liệu gửi đi (Khớp với RegisterRequest trong C#)
  const requestData = {
    username: username,
    password: password,
    fullName: fullName,
    phoneNumber: phone,
    role: role,
    address: role === "KhachHang" ? address : null,
    licensePlate: role === "Shipper" ? license : null,
  };

  try {
    // Gửi request lên API (Cổng 5000 hoặc cổng máy bạn đang chạy)
